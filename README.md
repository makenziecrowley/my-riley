...
2. Upload all the files in this folder (`index.html`, `love.html`, `support.html`, `jokes.html`, `games.html`, `ourgames.html`, `photos.html`, `style.css`, and the `assets` folder) to that repo. Easiest way: on the repo page, click "Add file" → "Upload files," then drag all of them in.
...
- Open `love.html` and personalize the letter with your own memories and words.
- Open `jokes.html` and edit the `quiz` array near the bottom (look for "EDIT ME") to swap in real questions about you two.
- Open `ourgames.html` and add real details to each game card (server names, usernames, records, inside jokes).
- Open `photos.html` and replace the placeholder boxes with real photos once you've picked them, by adding them to the `assets` folder and swapping in `<img src="assets/yourfile.jpg">`.
- Open `timeline.html`, `reasons.html`, `bucketlist.html`, `playlist.html`, `thisorthat.html`, `comfortjar.html`, and `secret.html` &mdash; every one of these has bracketed placeholder text like `[your words here]` that needs to be replaced with your real, personal writing. This is the bulk of the personalization work.
...
`secret.html` is linked from a locked envelope on the homepage. It only unlocks (becomes clickable) once Riley has visited every other content page on the same device/browser &mdash; love, support, jokes, games, ourgames, photos, timeline, reasons, bucketlist, playlist, thisorthat, and comfortjar. The homepage shows him a live count like "(8 / 12 opened)" so he knows there's more to find. Write the most personal message on the whole site in `secret.html` &mdash; it's the payoff for going through everything.
